...
export async function saveDiscoveryDraft(draft: DiscoveryDraft, operatorId: EventContact['capturedBy']): Promise<DiscoveryInterview> {
  ...
  const interview: DiscoveryInterview = {
    ...
    business_type: draft.business_type ?? existing?.business_type,
    business_category: draft.business_category ?? existing?.business_category,
    team_size: draft.team_size ?? existing?.team_size,
    ...
    owner_role: draft.owner_role ?? existing?.owner_role,
    owner_manual_tasks: draft.owner_manual_tasks ?? existing?.owner_manual_tasks,
    manual_control_hours: draft.manual_control_hours ?? existing?.manual_control_hours,
    critical_info_search: draft.critical_info_search ?? existing?.critical_info_search,
    ...
    growth_difficulties: draft.growth_difficulties ?? existing?.growth_difficulties ?? [],
    growth_difficulties_other: draft.growth_difficulties_other ?? existing?.growth_difficulties_other,
    ...
    margin_components: draft.margin_components ?? existing?.margin_components ?? [],
    ...
    stock_owner: draft.stock_owner ?? existing?.stock_owner,
    stock_sync: draft.stock_sync ?? existing?.stock_sync,
    stockout_frequency: draft.stockout_frequency ?? existing?.stockout_frequency,
    supplier_lead_time: draft.supplier_lead_time ?? existing?.supplier_lead_time,
    supplier_count: draft.supplier_count ?? existing?.supplier_count,
    uses_full: draft.uses_full ?? existing?.uses_full,
    ads_usage: draft.ads_usage ?? existing?.ads_usage,
    ads_manager: draft.ads_manager ?? existing?.ads_manager,
    ads_budget_method: draft.ads_budget_method ?? existing?.ads_budget_method,
    ads_profitability: draft.ads_profitability ?? existing?.ads_profitability,
    supply_models: draft.supply_models ?? existing?.supply_models ?? [],
    primary_supply_model: draft.primary_supply_model ?? existing?.primary_supply_model,
    supply_model_other: draft.supply_model_other ?? existing?.supply_model_other,
    post_sale_channels: draft.post_sale_channels ?? existing?.post_sale_channels ?? [],
    recurring_postsale_issue: draft.recurring_postsale_issue ?? existing?.recurring_postsale_issue,
    open_problem: draft.open_problem ?? existing?.open_problem,
    missing_data: draft.missing_data ?? existing?.missing_data,
