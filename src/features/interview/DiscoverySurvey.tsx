...
export default function DiscoverySurvey({ onExit, operator, resume = false }: DiscoverySurveyProps) {
  ...
  useEffect(() => { if (!resume) return; let active = true; void listDiscoveryInterviews().then((items) => { const latest = items.find((item) => item.status === 'in_progress' && item.operatorId === operator); if (active && latest) { setDraft(latest); setStep(Math.min(5, latest.currentStep)); setSavedId(latest.id) } }); return () => { active = false } }, [operator, resume])
